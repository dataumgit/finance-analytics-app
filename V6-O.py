...
class FinancialAnalyticsApp:
    ...
    def create_stock_info_card(self, parent):
        ...
        for label_text in ["Open:", "Day High:", "Day Low:", "Volume:"]:
            ...
            ttk.Label(sub_frame, text=label_text, font=('Arial', 10, 'bold'),
                      style="StockInfo.TLabel").pack(anchor=tk.W)
            key = label_text.lower().replace(" ", "").replace(":", "")
            self.stock_info[key] = ttk.Label(sub_frame, text="--", font=('Arial', 12),
                                             style="StockInfo.TLabel")
            ...
            if key == "sma":
                ...
                try:
                    if len(data) > 14:
                        ...
                        ax2.plot(data.index, rsi_data, color='orange', linewidth=1.5, label='RSI (14)')
                        ax2.set_ylim([0, 100])
    ...
    def update_historical_table(self):
        ...
        for i in range(num_rows):
            ...
            self.hist_tree.insert("", tk.END,
                                  values=(date_str, open_price, high_price, low_price, close_price, volume))
    ...
    def update_recommendation(self):
        ...
        try:
            ...
            rsi_val = calculate_rsi(self.stock_data).iloc[-1]

            # 生成简单的推荐逻辑
            if current_price > sma_20 and (sma_50 is None or current_price > sma_50) and rsi_val < 70:
                recommendation = "BUY"
                rec_tag = "positive"
                reasoning = [
                    "Stock is trading above moving averages, indicating a positive trend.",
                    f"RSI of {rsi_val:.1f} suggests the stock is not overbought yet.",
                    f"The stock gained {perf_30d:.1f}% over the last 30 days."
                ]
            elif current_price < sma_20 and (sma_50 is None or current_price < sma_50) and rsi_val > 30:
                recommendation = "SELL"
                rec_tag = "negative"
                reasoning = [
                    "Stock is trading below moving averages, indicating a negative trend.",
                    f"RSI of {rsi_val:.1f} suggests the stock is not oversold yet.",
                    f"The stock changed by {perf_30d:.1f}% over the last 30 days."
                ]
            elif rsi_val < 30:
                ...
                reasoning = [
                    ...
                    "Trend might continue, but this could be a dip-buy point."
                ]
            elif rsi_val > 70:
                ...
                reasoning = [
                    ...
                    "Momentum could continue, but risk is higher."
                ]
            else:
                recommendation = "HOLD"
                rec_tag = "neutral"
                reasoning = [
                    ...
                    f"RSI of {rsi_val:.1f} is neutral.",
            ...
            if rsi_val > 70:
                self.rec_text.insert(tk.END, f"{rsi_val:.2f} (Overbought)\n", "negative")
            elif rsi_val < 30:
                self.rec_text.insert(tk.END, f"{rsi_val:.2f} (Oversold)\n", "positive")
            else:
                self.rec_text.insert(tk.END, f"{rsi_val:.2f} (Neutral)\n", "neutral")
            ...
            self.rec_text.insert(tk.END,
                                 "\nDisclaimer: This recommendation is based solely on technical analysis "
                                 "and does not take into account fundamental factors. This is not financial advice.")
