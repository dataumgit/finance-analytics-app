...
class FinanceAnalyticsApp:
    def __init__(self, root):
        ...
        ttk.Checkbutton(self.indicators_frame, text="简单移动平均线 (SMA)", variable=self.sma_var).pack(anchor=tk.W,
                                                                                                        padx=5, pady=2)
        ttk.Checkbutton(self.indicators_frame, text="相对强弱指数 (RSI)", variable=self.rsi_var).pack(anchor=tk.W,
                                                                                                      padx=5, pady=2)
        ttk.Checkbutton(self.indicators_frame, text="布林带 (Bollinger Bands)", variable=self.bb_var).pack(anchor=tk.W,
                                                                                                           padx=5,
                                                                                                           pady=2)
        ttk.Checkbutton(self.indicators_frame, text="MACD", variable=self.macd_var).pack(anchor=tk.W, padx=5, pady=2)
        ttk.Checkbutton(self.indicators_frame, text="成交量分析", variable=self.vol_var).pack(anchor=tk.W, padx=5,
                                                                                              pady=2)
